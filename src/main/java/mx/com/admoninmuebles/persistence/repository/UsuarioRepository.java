...
//	boolean existByUsername( String userName );
    ...
    @Modifying
    @Query(value = "delete from gescopls.inmuebles_socios\n" + 
			"where socios_id_usuario = ?1", 
			nativeQuery = true)
    void deleteInmuelesSociosBySocioId(Long id);
    
    @Query(value = "select count(socios_id_usuario) from gescopls.inmuebles_socios\n" + 
			"where socios_id_usuario = ?1", 
			nativeQuery = true)
    Long countInmuelesSociosBySocioId(Long id);
